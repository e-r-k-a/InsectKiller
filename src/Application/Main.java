...
public class Main extends JFrame implements ActionListener, MenuListener {
	...
	public Main() {
		...
		tabMeasurments.setModel(new DefaultTableModel(
			new Object[][] {
				{null, null, null},
			},
			new String[] {
				"L.p.", "Nazwa czujnika", "Warto\u015B\u0107"
			}
		) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(source == btnStop){
			if((mode == MODE_HEATING)||(mode == MODE_PAUSE)){
				...
				btnPause.setBackground(Color.LIGHT_GRAY);
				LocalTime l = LocalTime.now();
				textRaport.append(l.getHour() + ":"+l.getMinute() + ":"+l.getSecond() + " - Wyłączone grzanie - nie osiągnięta temperatura\n");//wpisanie do zdarzeń
			}
		}
		if(source == btnPause){
			if(mode == MODE_HEATING){
				...
				LocalTime l = LocalTime.now();
				textRaport.append(l.getHour() + ":"+l.getMinute() + ":"+l.getSecond() + " - Proces grzania wstrzymany\n");//wpisanie do zdarzeń
			}
		}
	}
	...
	public static void main(String[] args) {
		...
		while(true) {
			try {
					...
					w.textArea.append(LocalDate.now().toString() + " " + now.getHour() + ":" + now.getMinute() + ":" + now.getSecond() + "\n");
					// i na konsole
					System.out.format("zad_reg=%.2f zad_ogr=%.2f temp min= %.2f temp max=%.2f uchyb reg.=%.2f wyjscie= %.2f tryb reg.=%.0f tryb ogr.= %.0f kp=%.2f Tc=%.2f kp_lim=%.2f Tc_lim=%.2f\n",w.zadTemp, w.maxTemp, actualMinimal, actualMaximal, controllerError, output, pidController.getMode(), pidLimiter.getMode(), w.kp, w.Tc, w.limKp, w.limTc);
					//wpisanie wartości*/
